...
if [ ${PRODUCTVERSION%%.*} -le 12 ]; then
    cp ${HOMEBREWDIR}/opt/libffi/lib/libffi.8.dylib lib
fi
...
if [ ${PRODUCTVERSION%%.*} -le 12 ]; then
    install_name_tool -id "homebrew:libffi/lib/libffi.8.dylib" lib/libffi.8.dylib
fi
...
if [ ${PRODUCTVERSION%%.*} -le 12 ]; then
    install_name_tool -change ${HOMEBREWDIR}/opt/libffi/lib/libffi.8.dylib @executable_path/lib/libffi.8.dylib lib/libp11-kit.0.dylib
fi
